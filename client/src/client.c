...
static int getCurrentTimeinfo(struct tm **timeinfo)
{
    ...
    *timeinfo = localtime(&rawtime);
    ...
    fprintf(stderr,
            "Current date and time: " WHITE "%s" NC, asctime(*timeinfo));
...
int main(void)
{
    ...
    fprintf(stderr, "Set current date to %d.%d.%d\n",
            KOS_BIRTHDAY_DAY, KOS_BIRTHDAY_MONTH, KOS_BIRTHDAY_YEAR);
...
void error( char* msg );
int time_get_and_print( char* host_name );

// NTP server host-name.
char* host_name1 = "us.pool.ntp.org"; 
char* host_name2 = "pool.ntp.org";
...
int time_get_and_print( char* host_name )
{
  ...
  ssize_t n; // the n return result from writing/reading from the socket.
  ...
  *( ( char * ) &packet + 0 ) = 0x1b; // Represents 27 in base 10 or 00011011 in base 2.
  ...
  fprintf(stderr, "Time: " WHITE "%s" NC, ctime( ( const time_t* ) &txTm ));
...
//    sleep(5);
    ...
    fprintf(stderr, "Host: " GREEN "%s" NC, host_name1);
    time_get_and_print( host_name1 );
//    sleep(2);
    fprintf(stderr, "Host: " GREEN "%s" NC, host_name2);
    time_get_and_print( host_name2 );
